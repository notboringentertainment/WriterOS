...
- **Loop + terminal tool.** Read tools may run during the loop; the model **must** close by calling `respond_to_writer({ message, suggestions })`. Single structured response path. (Post-review amendment: `receipts`/`limits` were removed from the M1 terminal schema — there is no response/render path for them yet; they return in M2 with `askSpecialist` + the receipt UI, wired end-to-end.)
- **One assistant message per turn, one combined tool_result turn** (Anthropic history rule): when a turn calls multiple tools, push `assistantTurn(content)` once and a single user message carrying every matching `tool_result`. (Post-review fix; M2's multi-tool turns depend on it.)
