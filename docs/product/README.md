...
- `docs/product/unified-project-memory-prd.md` is canonical for Unified Project Memory V1: the local append-only archive inside a `.writeros` package; source evidence, document facts, canon authority and supersession, conflict arbitration, retrieval, reporting, workflow records, privacy, and V1 non-goals. It requires Ben's arbitration for Buzz canon conflicts and does not adopt Buzz's native auto-win rule. Operational recovery for this system (rebuilding a corrupted snapshot, diagnosing a corrupt ledger, retrying a partial import, reverting a hand-reviewed `integrations/` contract edit, and disabling shared memory for a browser-only project) is `docs/runbooks/unified-project-memory.md`, not this doc.
