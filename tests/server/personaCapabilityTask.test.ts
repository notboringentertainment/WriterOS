...
describe('parseResearchTaskResult', () => {
  it('normalizes structured upstream research with sources', () => {
    ...
    expect(result.findings[0]).toMatchObject({ claim: 'The current gate dates to the Ottoman period.', verified: true })
    expect(result.findings[1]).toMatchObject({ verified: false })
