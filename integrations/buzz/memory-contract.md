...
to anchor the edit below; 525 lines, ending with "§11. Canon lock routing (addendum,
2026-08-05)").
...
4. **Generate only where atoms are locally readable; deliver manually.** `atoms/`, `traces/`,
   ...
   Studio → Air, and that rsync carries evidence locators, never atom bodies or story
   content (see the project's household notes on the archiver: raw vault events never leave
   Ben-controlled machines). This contract adds no transport. The bundle is generated by
   hand on whichever machine has `atoms/` locally readable (today, the Studio), then handed
   to the requesting agent directly — paste, attach, or point at the path. There is no
   channel, no sync job, no automatic fetch, and no assumption that a bundle generated on
...
today — only the L0 vault (raw relay events) rsyncs Studio → Air, and that rsync
carries evidence locators, never atom bodies or story content. This addendum adds
no transport. It is a bundle-generation procedure that runs by hand, on the one
machine where these directories are locally readable (currently the Studio),
followed by manual delivery of the resulting file to whichever agent needs it.
