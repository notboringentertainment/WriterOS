// =============================================================================
//  ui/frame.jsx  ·  Presentation layer scaffold
// -----------------------------------------------------------------------------
//  Renders the 3-pane app chrome:
//     ┌─ TopBar ───────────────────────────────────────────────────────┐
//     │ project · active agent · sync · system status                  │
//     ├─────────────┬────────────────────────────────────┬─────────────┤
//     │  LeftNav    │            Main canvas              │ RightDrawer│
//     │  (sections) │            (current screen)         │ (inspector)│
//     └─────────────┴────────────────────────────────────┴─────────────┘
//
//  All state access goes through window.WOS.store. Components do not fetch.
// =============================================================================

const { store } = window.WOS;

// --- hook: subscribe to store, re-render on change ----------------------------
...
// --- TopBar -------------------------------------------------------------------
function TopBar({ screen, setScreen, activeAgentId }) {
    ...
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{
          ...
          <svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="#0f0b13" strokeWidth="2" strokeLinecap="round"><path d="M3 13L13 3M10 3h3v3"/></svg>
        </div>
...
// --- LeftNav ------------------------------------------------------------------
function LeftNav({ screen, setScreen }) {
  const agents = useStore(s => s.agents);
...
// --- RightDrawer (context-aware inspector) -----------------------------------
function RightDrawer({ screen, activeAgentId }) {
  ...
  // decide what "context" means right now
  let contextBlock;
  if (activeAgent) {
...
function EventStream() {
  ...
  React.useEffect(() => {
    return window.WOS.eventBus.on('*', ({ event, payload }) => {
