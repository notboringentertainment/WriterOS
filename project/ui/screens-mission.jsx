// =============================================================================
//  ui/screens-mission.jsx
//  Mission Control · Triage · Tasks · Handoffs
// =============================================================================

function MissionControl({ setScreen }) {
...
function HandoffCard({ h, agents }) {
  ...
  return (
    <div style={{
      minWidth: 260, flexShrink: 0, padding: '10px 12px', borderRadius: 6,
      background: 'var(--bg)', border: '1px solid var(--border)',
    }}>
...
function Dot({ a }) { return <span style={{ width: 16, height: 16, borderRadius: 8, background: a.accent, color:'#15120e', display:'flex', alignItems:'center', justifyContent:'center', fontSize: 9, fontWeight: 700 }}>{a.letter}</span>; }
function Arrow()   { return <svg width="12" height="8" viewBox="0 0 12 8" fill="none" stroke="var(--fg-muted)" strokeWidth="1.2" strokeLinecap="round"><path d="M1 4h10m-3-3l3 3-3 3"/></svg>; }

// ============================================================================
//  Triage — the coordinator's view.
// ============================================================================

function TriageView({ setScreen }) {
  const agents   = useStore(s => s.agents);
  const tasks    = useStore(s => s.tasks);
  const inbox    = tasks.filter(t => t.assignedTo === null);
  const blocking = useStore(s => s.store?.sel?.blockingEdges?.() || window.WOS.store.sel.blockingEdges());
            ...
            <div key={t.id} style={{ padding: '10px 12px', borderRadius: 6, background: 'var(--bg)', border: '1px solid var(--border)', marginBottom: 8 }}>
              ...
              <div style={{ marginTop: 8, display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                ...
                {agents.filter(a=>a.id!=='triage').map(a => (
                  <button key={a.id} onClick={() => window.WOS.store.actions.assignTask(t.id, a.id)} style={routeBtn(a.accent)}>
...
function NewTaskRow({ agents, onClose }) {
  ...
  const create = () => {
    ...
    window.WOS.store.actions.createTask({ title, assignedTo: assigned, state: assigned ? 'queued' : 'draft' });
    onClose();
  };
  return (
...
// ============================================================================
//  Shared primitives for screens
// ============================================================================

function Screen({ title, subtitle, actions, children }) {
  return (
...
function Panel({ span = 12, title, kicker, children }) {
  return (
...
function Chip({ children }) { return <span style={{ fontSize: 10, fontFamily: 'var(--mono)', padding: '2px 7px', borderRadius: 3, background: 'var(--chip)', color: 'var(--fg-muted)' }}>{children}</span>; }
function Empty({ children }) { return <div style={{ fontSize: 12, color: 'var(--fg-muted)', fontStyle: 'italic', padding: '20px 0', textAlign: 'center' }}>{children}</div>; }
...
Object.assign(window, {
  MissionControl, TriageView, TaskQueue, HandoffTimeline,
  Screen, Panel, Stat, Chip, Empty, primaryBtn, ghostBtn,
});
