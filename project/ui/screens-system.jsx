// =============================================================================
//  ui/screens-system.jsx
//  State inspector · Future modules · Settings
// =============================================================================

function StateInspector() {
  const all = useStore(s => s);
  const [path, setPath] = React.useState('');
  const [expanded, setExpanded] = React.useState({ project: true, agents: true, tasks: true });

  const slice = path ? get(all, path) : all;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: 14 }}>
        <Panel title="Schema" kicker="top-level keys">
          ...
          <div style={{ fontSize: 10, color: 'var(--fg-muted)', fontFamily:'var(--mono)', marginTop: 14, paddingTop: 10, borderTop: '1px solid var(--border)', lineHeight: 1.6 }}>
            ...
            <div>client/lib/schema.js defines shapes.</div>
...
function SyntaxJSON({ value }) {
  // simple, readable — not a full highlighter
...
function FutureModules({ setScreen }) {
  const mods = [
    ...
    <Screen title="Settings" subtitle="Runtime · storage · agent tuning">
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
        ...
        <Panel title="Agents" kicker="turn them on / off">
          {['sam','casey','oliver','maya','zoe','marcus'].map(id => {
            const a = window.WOS.store.sel.agent(id);
            return (
              <div key={id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: '1px solid var(--border)' }}>
...
Object.assign(window, {
  StateInspector, FutureModules, SettingsView,
});
