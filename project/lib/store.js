// =============================================================================
//  lib/store.js  ·  Client State Layer
// -----------------------------------------------------------------------------
//  Redux-lite, no deps. Wraps the canonical ProjectState and exposes:
//    • typed selectors for the UI
//    • mutations that MUST go through the bridge (never set state directly)
//    • subscription API used by useStore hook in app.jsx
//
//  Every component reads from here. Nothing reads from fetch/ws directly.
// =============================================================================

(function(){
  const { eventBus, bridge, MOCK_STATE } = window.WOS;

  let state = MOCK_STATE;
  const subs = new Set();
  const notify = () => subs.forEach(fn => fn(state));

  // Merge deep-ish patch onto state (dot-path key → value).
  ...
  // ------ mutations (all routed through bridge; optimistic patch allowed) ------
  const actions = {
