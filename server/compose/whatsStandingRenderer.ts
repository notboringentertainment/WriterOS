...
function recordBlocks(entry: StandingEntry): ComposedBlock[] {
  ...
  for (const cue of entry.cues) {
    if (quoted.has(cue.sentence)) continue
    quoted.add(cue.sentence)
    blocks.push({
      type: 'leadInParagraph',
      lead: 'This record points elsewhere:',
      text: `“${cue.sentence}” — this report does not resolve what that refers to.`,
      sourceFieldIds: [entry.record.id],
    })
  }
...
export function renderWhatsStandingBlocks(snapshot: ProjectMemorySnapshot): ComposedBlock[] {
  ...
  section('In force', entries.filter(e => e.group === 'inForce').flatMap(recordBlocks))
  ...
  section(
    ...
    entries.filter(e => e.group === 'withdrawn').flatMap(recordBlocks),
  )
